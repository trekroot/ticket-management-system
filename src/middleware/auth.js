...
export async function verifyUserAuthenticated(req, res, next) {
  ...
  try {
    ...
    // additional, maybe unnecessary protection
    // PROD only: Restrict Firebase auth to admin users only (defense in depth)
    if (process.env.NODE_ENV === 'prod' && result.provider === 'firebase' && result.user.role !== 'admin') {
      return res.status(403).json({
        error: 'Access denied. Please log in through the Dirigo Union website.'
      });
    }
